...
def update(request, id):
    ...
    if serializer.is_valid():
        if 'client_image' in request.FILES:
            serializer.validated_data['client_image'] = re.sub(r'_[^_]*\.', '.',
            request.FILES['client_image'].name)
        if 'document_image' in request.FILES:
            serializer.validated_data['document_image'] = re.sub(r'_[^_]*\.', '.',
            request.FILES['document_image'].name)
        serializer.save()
